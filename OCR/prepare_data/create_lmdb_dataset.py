...
def createDataset(inputPath, gtFile, outputPath, checkValid=True):
    ...
    gtFile = os.path.abspath(gtFile)
    outputPath = os.path.abspath(outputPath)
    
    os.makedirs(outputPath, exist_ok=True)
    env = lmdb.open(outputPath, map_size=2 * 1024 * 1024 * 1024)
    cache = {}
    cnt = 0
    ...
    for i in tqdm.tqdm(range(nSamples)):
        ...
        label = covert_unicode(label)
        label = re.sub('°', '', label)
        label = re.sub('_', '-', label)
        label = re.sub('̣' , '', label)
        label = re.sub('̉' ,'', label)
...
if __name__ == '__main__':
    ...
    createDataset("../data/train_images", "../data/annotation_train.txt",'../model/train_lmdb') 
    createDataset("../data/valid_images", "../data/annotation_valid.txt",'../model/valid_lmdb') 